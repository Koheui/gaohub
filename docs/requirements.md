...
/dashboard/events/[id]/surveys  … アンケート一覧・作成(質問/対象/送信タイミング)
/dashboard/events/[id]/surveys/[surveyId] … アンケート詳細・回答集計・今すぐ送信/再送信
/e/[slug]/survey/[surveyId]     … 参加者向けアンケート回答ページ(?r=登録ID&k=qrToken)
/api/events/[id]/surveys/[surveyId]/send    … 今すぐ送信(org メンバー検証)
/api/events/[id]/surveys/[surveyId]/respond … 回答受付(チケットリンクで本人確認)
/api/cron/surveys       … 予約送信の処理(status=scheduled かつ scheduledAt<=now を送信。
                          本番は Vercel Cron 等から定期実行。CRON_SECRET で保護)
...
/t/[ticketId]           … QRチケット表示(メールのリンク先)。セッション予約と
                          コミュニティラウンジへの入口もここに表示
/t/[ticketId]/lounge    … コミュニティラウンジ専用ページ(登壇者・参加者の一覧、
                          カテゴリ絞り込み、メッセージ送信、プロフィール編集・退出)
...
イベント全体・セッションのどちらも同じアート系デザイン(タイポポスター/モノクロ/
スプリット2色/デュオトーン/ジオメトリック)を選べる — イベント全体バナーは
セッションレンダラーを疑似セッション(タイトル=イベント名・会場=venueName・
登壇者=全員)で流用する(`renderEventArtisticBanner`)。
選んだデザインは「OG共有画像に設定」で `EventDoc.bannerStyle` に保存され、
LP を SNS 共有した際のカード画像(`opengraph-image`)に反映される。
...
- **予約は申込時に確定させる必要はなく、後からいつでも変更できる**。
  チケットページが参加者のマイページを兼ね、開催当日まで予約の追加・取消が可能
  (UI上にもその旨を明記)。申込完了メールを失った参加者のために
  **マイチケット再送**(`/e/[slug]/my-ticket` → `/api/tickets/resend`)を用意 —
  申込時のメールアドレスを入力するとチケットリンクを再送する。登録の有無に
  かかわらず同じレスポンスを返し、メールアドレスの存在を漏らさない。
  LPのチケット欄・申込フォームから「マイチケット」へリンクする
...
チケット購入者が登壇者・参加者と交流できる**専用ラウンジページ**
(`/t/[registrationId]/lounge?k=…`)。チケットページはコンパクトな入口
(参加状況+「ラウンジへ入る →」ボタン)のみを置く。設計上のポイント:

- **参加導線は2つ**: ①申込フォームの「コミュニティラウンジに参加する」
  チェックボックス(`Registration.joinLounge`) — 確定時に申込情報(氏名・会社・役職)
  からプロフィールを自動作成する(無料チケットは即時、有料は Stripe webhook の
  確定時)。②後からラウンジページの参加フォームで登録
- **オプトイン制**: 登録しなくてもチケット・予約機能は問題なく使える。
  プロフィール(表示名・会社・肩書・カテゴリ・自己紹介)はラウンジページで
  いつでも編集でき、退出すれば即削除される
- **登壇者もラウンジに表示される**: 写真・肩書つきで一覧され(SPEAKERバッジ)、
  主催者が登壇者管理で**非公開メールアドレス**(`SpeakerDoc.email`。公開ページや
  APIには一切出さない)を設定した登壇者には参加者からメッセージを送れる
  (`/api/lounge/contact` の `toSpeakerId`)。登壇者はメールの返信でそのまま
  会話を継続できるため、登壇者側のログインは不要
- **カテゴリは主催者が定義**: イベント編集フォームの「コミュニティラウンジ」欄で
  タグを自由に追加(例: 運営者/投資家/支援者/スタートアップ)。ラウンジページは
  カテゴリチップで参加者を絞り込める
- **メールアドレスは公開しない**: 一覧API(`/api/lounge/directory`)は
  プロフィール項目のみを返す(登壇者は「連絡可能か」のフラグのみ)。
  メッセージ送信はサーバー側で受信者のメールを解決して Resend 経由で送るのみで、
  クライアントには一切返さない。送信者のメールは`replyTo`に設定するため、
  受信者が返信すれば通常のメールとして会話を継続できる
...
参加者向けの申込フォームに何を入力させるかは、イベントごとにダッシュボードから
設定できる(チケット管理画面 `/dashboard/events/[id]/tickets` の
「申込フォームの質問項目」)。

- **氏名・メールアドレスは常時必須・表示固定**(QRチケットの送付に必要なため、
  非表示・任意化はできない)
- **会社名・役職は表示有無・必須有無をイベントごとに切り替え可能**
  (`EventDoc.askCompany` / `requireCompany` / `askJobTitle` / `requireJobTitle`)。
  例えば toC向けの一般参加者向けイベントでは会社名欄自体を非表示にできる
- **会社名・役職は入力形式も自由入力/プルダウンから選べる**
  (`companyFieldType` / `jobTitleFieldType` が `"text"` または `"select"`、
  選択肢は `companyFieldOptions` / `jobTitleFieldOptions` にカンマ区切りで設定)。
  役職を「経営者/管理職/一般社員/学生」のような定型選択にしたい場合などに使う
- **追加のカスタム質問**は主催者が自由に定義できる(`EventDoc.registrationFields`)。
  種類は短文(text)/ 長文(textarea)/ 選択式(select、選択肢はカンマ区切りで入力)/
  チェックボックス(checkbox)。必須/任意を項目ごとに設定可能
- 上記のバリデーション(会社名・役職の必須設定を含む)はクライアント側と
  サーバー側(`/api/checkout`)の両方でおこなう。カスタム質問の回答は
  `Registration.customAnswers`(質問IDをキーとするマップ)に保存され、
...
主催者へのデータフィードバックを価値の中心に据える。`/api/events/[id]/report`
(ID トークン + org メンバー検証)で Admin SDK による全件集計を行い、
ダッシュボードの「レポート」タブで可視化する。集計項目: 申込サマリ(確定/決済待ち/
キャンセル)、チェックイン率、売上、チケット種別内訳、所属(会社)・役職の分布、
申込フォームのカスタム質問(選択式/チェックボックス)の集計、セッション予約数による
人気コンテンツランキング、ラウンジ参加率、日別申込推移。ダッシュボードの
デザイン言語に合わせた横棒グラフで表示する。

### アンケート

主催者が参加者へメールでアンケートを送付できる。`events/{id}/surveys` に保存。

- **質問は申込フォームのカスタム質問と同じ形式**(`RegistrationFieldDef[]`。短文/長文/
  選択式/チェックボックス)。編集UIは共通コンポーネント `QuestionsEditor` を
  申込フォーム設定と共用する
- **送付対象(audience)**: 確定者全員 / 有料チケットのみ / 当日チェックイン済みのみ
- **送信タイミング**: 「今すぐ送信」/「日時を指定して予約」/「下書き保存」。
  予約送信は `status=scheduled` + `scheduledAt` を保存し、cron エンドポイント
  `/api/cron/surveys`(全イベント横断の collectionGroup クエリ)が
  `scheduledAt<=now` のものを送信する。本番は Vercel Cron 等から定期実行し
  `CRON_SECRET` で保護。ローカルは手動 GET で検証
- **回答ページ**は公開(`/e/[slug]/survey/[surveyId]?r=登録ID&k=qrToken`)。
  チケットリンクと同じ token で本人確認する。回答は
  `surveys/{id}/responses/{registrationId}` に保存(再回答で上書き)、
  ダッシュボードの詳細ページで選択式/チェックボックスは棒グラフ、
  自由記述は一覧で集計表示する
- メールは `sendSurveyEmail`(未設定時はコンソールログの既存パターン)
